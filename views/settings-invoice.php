...
try {
    ?>
    <h1 class="wp-heading-inline"><?=__('Invoice', 'onfact')?></h1>
    ...
    <form  method="post" action="options.php">
        ...
        <table class="form-table" role="presentation">
            ...
            <?PHP
            settings_fields('onfact-settings-invoice');
            ...
            foreach ($languages->getItems() as $language):
                ...
                ?>
                <?php do_settings_sections( 'onfact-settings-invoice' ); ?>
                <tr>
                    <th scope="row"><label for="api_key"><?=__('Description', 'onfact')?> <?=__($language->getName(), 'onfact')?></label></th>
                    <td>
                        <select name="invoice_description_<?=$language->getId()?>" id="description.<?=$language->getId()?>" class="regular-text">
                            <option value="none"><?=__('No Description', 'onfact')?></option>
                            <?PHP foreach ($descriptions->getItems() as $description): ?>
                                <?PHP if ($description->getLanguageId() !== $language->getId() || $description->getModel() !== 'Invoice') continue ?>
                                <option
                                        value="<?=$description->getId()?>"
                                    <?=$description->getId() == get_option('invoice_description_' . $language->getId()) ? 'selected=selected' : ''?>>
                                    <?=$description->getName()?>
                                </option>
                            <?PHP endforeach; ?>
                        </select>
        ...
        <table class="form-table" role="presentation">
            ...
            foreach ($languages->getItems() as $language):
                ...
                <tr>
                    ...
                    <td>
                        <select name="invoice_emaildescription_<?=$language->getId()?>" id="description.<?=$language->getId()?>" class="regular-text">
                            <option value="none"><?=__('Do not send email', 'onfact')?></option>
                            <?PHP foreach ($emaildescriptions->getItems() as $emaildescription): ?>
                                <?PHP if ($emaildescription->getLanguageId() !== $language->getId() || $emaildescription->getModel() !== 'Invoice') continue ?>
                                <option
                                        value="<?=$emaildescription->getId()?>"
                                    <?=$emaildescription->getId() == get_option('invoice_emaildescription_' . $language->getId()) ? 'selected=selected' : ''?>>
